...
func (l *FileLoader) loadCSVStreaming(ctx context.Context, filePath string) (*LoadResult, error) {
	...
	if len(emptyColumns) > 0 {
		...
		header, columnTypes = RemoveColumnsFromSchema(header, columnTypes, emptyColumns)
	}
	...
	for {
		...
		if err == io.EOF {
			if len(chunk) > 0 {
				if err := l.insertBatch(ctx, tableName, header, columnTypes, chunk, emptyColumns); err != nil {
...
func (l *FileLoader) insertBatch(ctx context.Context, tableName string, columns []string, types map[string]string, rows [][]string, emptyColumns map[int]bool) error {
	if len(rows) == 0 {
		return nil
	}

	// Calculate active columns (excluding empty ones)
	activeColumns := len(columns)
	if emptyColumns != nil {
		for range emptyColumns {
			activeColumns--
		}
	}
	...
	for startIdx := 0; startIdx < len(rows); startIdx += maxRowsPerBatch {
		...
		if err := l.executeBatch(ctx, tableName, columns, types, batch, emptyColumns); err != nil {
			return err
		}
	}
...
// executeBatch performs the actual INSERT query
func (l *FileLoader) executeBatch(ctx context.Context, tableName string, columns []string, types map[string]string, rows [][]string, emptyColumns map[int]bool) error {
	if len(rows) == 0 {
		return nil
	}

	// ⭐ BUILD ACTIVE COLUMN LIST (excluding empty columns)
	var activeColumns []string
	var activeColIndices []int

	for colIdx, col := range columns {
		// Skip empty columns
		if emptyColumns != nil && emptyColumns[colIdx] {
			continue
		}
		activeColumns = append(activeColumns, col)
		activeColIndices = append(activeColIndices, colIdx)
	}

	// If all columns are empty, return error
	if len(activeColumns) == 0 {
		return fmt.Errorf("no active columns to insert")
	}
	...
	for _, row := range rows {
		...
		// ⭐ ONLY ITERATE OVER ACTIVE COLUMNS
		for _, colIdx := range activeColIndices {
			col := columns[colIdx]
	...
	// ⭐ QUOTE ONLY ACTIVE COLUMNS
	quotedCols := make([]string, len(activeColumns))
