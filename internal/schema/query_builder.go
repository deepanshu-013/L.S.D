...
func (qb *QueryBuilder) EncodeCursor(tableName string, row map[string]interface{}, sortColumn string) string {
	...
	if sortColumn != "" && sortColumn != table.PrimaryKey[0] {
...
func (qb *QueryBuilder) BuildSelectQuery(params QueryParams) (*BuiltQuery, error) {
	...
	query := fmt.Sprintf(`
		...
		ORDER BY %s %s, %s %s
		LIMIT $%d
	`, strings.Join(columns, ", "), params.TableName, whereClause, sortBy, sortDir, pkOrder, sortDir, argIndex)
...
func (qb *QueryBuilder) buildCursorCondition(cursor *DynamicCursor, sortDir string, startArgIndex int) (string, []interface{}, int, error) {
	...
	if len(cursor.Values) == 1 {
		...
		condition := fmt.Sprintf("%s %s $%d", cv.Column, op, argIndex)
	...
	parsedVal1, err := parseValueFromCursor(cv1.Value, cv1.Type)
	if err != nil {
		return "", nil, argIndex, err
	}
	parsedVal2, err := parseValueFromCursor(cv2.Value, cv2.Type)
	if err != nil {
		return "", nil, argIndex, err
	}

	condition := fmt.Sprintf("(%s %s $%d OR (%s = $%d AND %s %s $%d))",
		cv1.Column, op, argIndex,
		cv1.Column, argIndex,
		cv2.Column, op, argIndex+1)
	args = append(args, parsedVal1, parsedVal2)
	argIndex += 2
...
func (qb *QueryBuilder) BuildSearchQuery(params QueryParams, searchColumn, searchTerm string) (*BuiltQuery, error) {
	...
	if cursor != nil && len(cursor.Values) > 0 {
		...
		whereConditions = append(whereConditions, fmt.Sprintf("%s > $%d", cv.Column, argIndex))
		args = append(args, parsedVal)
		argIndex++
	}
